    ...
    private func rebuildSublayers() {
        ...
        for run in runs {
            ...
            for index in 0 ..< glyphsCount {
                ...
                let layer = GlyphLayer(glyph: glyph, font: font, offset: CGPoint(x: -rect.origin.x, y: -rect.origin.y))
                layer.frame = CGRect(x: rect.minX + position.x, y: rect.minY + position.y, width: rect.width, height: rect.height)
                layers.append(layer)
            }
        }
